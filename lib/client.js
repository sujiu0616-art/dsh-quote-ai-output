...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function resolveSource(text) {
      try {
        const rt = _runtime;
        const nodes = (rt && rt.conversation && rt.conversation.nodes) || [];
        ...
        for (const node of nodes) {
          ...
          if (full && normalize(full).indexOf(needle) >= 0) {
            return { turn: node.turn, seq: node.seq, messageId: node.messageId };
          }
        }
      } catch {}
      return null;
    ...
    function addQuote(text) {
      ...
      const q = {
        ...
        timestamp: ts, sessionTitle, kind: 'assistant',
      };
    ...
    function QuoteDock(props) {
      ...
      const { useConversation, useSessions, useInput, inputActions, sessionId } = props;

      // 会话快照：来源识别 + 会话标题（新版消息快照由 useConversation 提供）
      const conversation = useConversation === undefined ? undefined : useConversation((s) => s);
      const sessionsSnap = useSessions === undefined ? undefined : useSessions((s) => s);
      const input = useInput((s) => s);

      // 模块级逻辑（浮动工具栏 / Enter 注入）在 React 之外运行，需要最新引用
      React.useEffect(() => {
        _runtime = { conversation, sessionsSnap, input, inputActions, sessionId };
      });
