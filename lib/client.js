...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function QuoteAction(props) {
      const { messageId, useConversation, useSession, useInput, inputActions, insertQuote } = props;
      const input = useInput((s) => s);
      // 新版 DSH 的消息快照由 useConversation 提供（useSession 改为会话状态）；旧版在 useSession —— 双版本兼容
      const conversation = useConversation === undefined ? undefined : useConversation((s) => s);
      const session = conversation === undefined && useSession !== undefined ? useSession((s) => s) : undefined;
      const nodes = (conversation && conversation.nodes) || (session && session.nodes) || [];
      const grab = () => {
