...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function expandText(text, turn, timestamp, sessionTitle, kind) {
      ...
      // 闭合标记：正文与新输入可能同文，需要明确边界（多条引用各自闭合）
    ...
    // 匹配键：去掉空白与 markdown 标记
    // （选区取的是渲染后文本，而消息原文含 ** ` # - 等标记，直接比对会失配）
    ...
    function resolveSource(text) {
      try {
        ...
        // 倒序遍历（最新优先）：短文本或常见短语命中多条时，取最近的一条
        for (let i = nodes.length - 1; i >= 0; i--) {
          const node = nodes[i];
          if (node.kind === 'assistant') {
            const key = matchKey(messageTextOf(node));
            if (key && key.indexOf(needle) >= 0) {
              return { kind: 'assistant', turn: node.turn, seq: node.seq, messageId: node.messageId };
            }
          } else if (node.kind === 'user') {
            const key = matchKey(userTextOf(node));
            if (key && key.indexOf(needle) >= 0) {
